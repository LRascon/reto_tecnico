...
if datos:
    print("Datos obtenidos correctamente:", datos)
else:
...
def obtener_respuestas_contestadas_no_contestadas(datos):
    contestadas = 0
    no_contestadas = 0
    for respuesta in datos['items']:
        if respuesta.get('is_answered'):
            contestadas += 1
        else:
            no_contestadas += 1
...
def obtener_respuesta_menor_vistas(datos):
    respuesta_menor_vistas = None
    menor_vistas = float('inf')
    for respuesta in datos['items']:
        vistas = respuesta.get('view_count')
        if vistas is not None and vistas < menor_vistas:
            respuesta_menor_vistas = respuesta
            menor_vistas = vistas
    return respuesta_menor_vistas

def obtener_respuesta_mas_antigua_y_mas_reciente(datos):
    respuestas = datos['items']
    respuestas.sort(key=lambda x: x['creation_date'])  # Ordenar por fecha de creación
    respuesta_mas_antigua = respuestas[0]
    respuesta_mas_reciente = respuestas[-1]
    return respuesta_mas_antigua, respuesta_mas_reciente

def obtener_owner_con_mayor_reputacion(datos):
    owner_con_mayor_reputacion = None
    mayor_reputacion = 0
    for respuesta in datos['items']:
        owner = respuesta.get('owner')
        if owner and owner['reputation'] > mayor_reputacion:
            owner_con_mayor_reputacion = owner
            mayor_reputacion = owner['reputation']
...
print("Respuesta con menor número de vistas:", respuesta_menor_vistas)

respuesta_mas_antigua, respuesta_mas_reciente = obtener_respuesta_mas_antigua_y_mas_reciente(datos)
print("Respuesta más antigua:", respuesta_mas_antigua)
print("Respuesta más reciente:", respuesta_mas_reciente)

owner_con_mayor_reputacion = obtener_owner_con_mayor_reputacion(datos)
print("Owner con mayor reputación:", owner_con_mayor_reputacion)
'''
# Conexión a enlace
import requests
